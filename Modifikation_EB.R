...
rownames(costMatrix_AF) <-
  levels(lw_et_merged_with_all$mit_auffaelligkeiten)
colnames(costMatrix_AF) <-
  levels(lw_et_merged_with_all$mit_auffaelligkeiten)

### -------------------------------- Baum fitten
formula_AF <-
  formula(
    mit_auffaelligkeiten ~ laktationsnummer + zellzahl + schalmtest + schalmtest_stufe_3 + jahreszeit +
...
cM_AF_all <-
  confusionMatrix(prediction_AF_all,
                  lw_et_merged_with_all$mit_auffaelligkeiten)

### -------------------------------- Baum printen
rpart.plot(
  tree_AF_all,
  main = "KKB zur Identifikation von Modifikationen - Zielgröße \"Auffälligkeiten auf dem Baum\"",
  roundint = FALSE,
  extra = 1,
  box.palette = list('plum3', 'gold2'),
  fallen.leaves = T
)
legend(
  x = 0,
  ...
  legend = c("Ja", "Nein"),
  fill = c("plum3", "gold2"),
  title = "Klassifikation in den Blättern",
  xpd = TRUE,
  bty = "o",
  xjust = 0
)
legend(
  x = 0.73,
  y = 1.07,
  legend = c("Linker Wert: Anzahl \"ja\"", "Rechter Wert: Anzahl \"nein\""),
  #fill = NA,
  title = "Aufbau Knoten",
  xpd = TRUE,
  bty = "o",
...
lw_et_merged_with_all$MBU_AB <-
  as.factor(lw_et_merged_with_all$MBU_AB)
...
formula_MBU_AB <-
  formula(
    MBU_AB ~ laktationsnummer + zellzahl + schalmtest + schalmtest_stufe_3 + jahreszeit +
      mean_milch + diff_milch + last_milch + trend_zz + var_milch
  )
tree_MBU_AB <-
  rpart(
    formula = formula_MBU_AB,
    method = "class",
    data = lw_et_merged_with_all,
    maxdepth = 4,
    parms = list(loss = costMatrix_AF, split = "gini")
...
cM_MBU_AB <-
  confusionMatrix(prediction_MBU_AB, lw_et_merged_with_all$MBU_AB)

### -------------------------------- Baum printen
rpart.plot(
  tree_MBU_AB,
  main = "KKB zur Identifikation von Modifikationen - Zielgröße \"Ergebnis der MBU\"",
  roundint = FALSE,
  extra = 1,
  box.palette = list('plum3', 'gold2'),
  fallen.leaves = T
...
legend(
  x = 0.62,
  y = 1.07,
  legend = c("MBU positiv", "MBU negativ/Ohne Befund"),
  fill = c("plum3", "gold2"),
  title = "Klassifikation in den Blättern",
  xpd = TRUE,
  bty = "o",
...
legend(
  ...
  y = 0.9 ,
  legend = c(
    "Linker Wert: Anzahl \"major\"",
    "Rechter Wert: Anzahl \"minor\\ohne Befund\""
  ),
  #fill = NA,
  title = "Aufbau Knoten",
  xpd = TRUE,
...
formula_AB <-
  formula(
    antibiotikagabe ~ laktationsnummer + zellzahl + schalmtest + schalmtest_stufe_3 + jahreszeit +
      mean_milch + diff_milch + last_milch + trend_zz + var_milch
  )
tree_AB <-
  rpart(
...
rpart.plot(
  tree_AB,
  main = "KKB zur Identifikation von Modifikationen - Zielgröße \"Antibiotikagabe\"",
  roundint = FALSE,
  extra = 1,
  box.palette = list('plum3', 'gold2')
)
