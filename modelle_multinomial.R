...
lw_et_merged_af <-
  lw_et_merged_with_all[lw_et_merged_with_all$verlaufsklasse == 6,]
...
pred_af <-
  data.frame(
    ...
    "5" = rep(0, l_af),
    "6" = rep(1, l_af),
    prob_antibiotikagabe = rep(1, l_af),
    antibiotikagabe = rep(1, l_af)
  )
colnames(pred_af) <-
  c("1",
    "2",
    ...
    "5",
    "6",
    "prob_antibiotikagabe",
    "antibiotikagabe")

### Performance für multinomiales Modell ohne zufälligem Betriebseffekt
cross_validation_multinom_final <-
  function(k, formula, data, target) {
    set.seed(123123)
    cv_f1 <- rep(0, k)
    cv_accuarcy <- rep(0, k)
    ...
    for (i in 1:k) {
      ...
      cv_macro_f1[i] <-
        2 * (sum(cm$byClass[, 5], na.rm = T) / 5 * sum(cm$byClass[, 6], na.rm = T) /
               5) / (5 / sum(cm$byClass[, 5], na.rm = T) + 5 / sum(cm$byClass[, 6], NA.rm = T))
      ...
      pred_probs$antibiotikagabe <-
        ifelse(pred_class %in% c(1, 2), "nein", "ja")
      cv_auc_binary[i] <-
        auc(roc(test$antibiotikagabe, pred_probs$prob_antibiotikagabe))
      cm_binaer <-
        confusionMatrix(as.factor(pred_probs$antibiotikagabe),
                        test$antibiotikagabe)
      ...
      test_global_antibiotikagabe <-
        c(test$antibiotikagabe, lw_et_merged_af$antibiotikagabe)
      pred_probs_global <-
        add_column(pred_probs, "6" = rep(0, nrow(pred_probs)), .after = "5")
      pred_probs_global <- rbind(pred_probs_global, pred_af)
      cv_auc_global[i] <-
        multiclass.roc(test_global, pred_probs_global[,  c("1", "2", "3", "4", "5", "6")])$auc[1]
      cv_auc_binary_global[i] <-
        auc(roc(
          test_global_antibiotikagabe,
          pred_probs_global$prob_antibiotikagabe
        ))
    }
    c(
      Reduce(paste, deparse(formula)),
      mean(cv_auc, na.rm = T),
      mean(cv_auc_global, na.rm = T),
      mean(cv_auc_binary, na.rm = T),
      mean(cv_auc_binary_global, na.rm = T),
      mean(cv_macro_f1, na.rm = T),
      mean(cv_accuarcy_balanced, na.rm = T),
      mean(cv_accuarcy, NA.rm = T),
      mean(cv_kappa, NA.rm = T),
      mean(cv_accuracy_bin, na.rm = T),
      mean(cv_accuracy_balanced_bin, na.rm = T),
      mean(cv_f1_bin, na.rm = T),
      mean(cv_sens_bin, na.rm = T),
      mean(cv_spec_bin, na.rm = T)
    )
  }

### Performance für multinomiales Modell mit zufälligem Betriebseffekt
cross_validation_multinom_rnd_effect_final <-
  function(k, formula, data, target) {
    set.seed(123123)
    cv_f1 <- rep(0, k)
    cv_accuarcy <- rep(0, k)
    ...
    for (i in 1:k) {
      ...
      test <- filter(data, fold == i)
      fitted_mod <-
        mblogit(formula, data = train, random = ~ 1 | betrieb)
      pred_probs <-
        predict(fitted_mod, type = "response", newdata = test)
      pred_class <-
        factor(colnames(pred_probs)[apply(pred_probs, 1, which.max)],
               levels = c("1", "2", "3", "4", "5"))
      cm <- confusionMatrix(pred_class, test[, target])
      cv_accuarcy[i] <- cm$overall[[1]]
      cv_accuarcy_balanced[i] <- sum(cm$byClass[, 6], na.rm = T) / 5
...
final_model_multi_formula <-
  formula(df_overview_ordered_auc[1, "formula"])

# ---------------------------- Bestes Modell schätzen (Referenzkodierung)
final_model_multi <-
  multinom(final_model_multi_formula,
           data = lw_et_merged)
# Summary des Modells ausgeben
summary(final_model_multi)
...
performance_final_model_multi <-
  data.frame(
    ...
    "cv_auc_global" = c(8),
    "cv_auc_binaer" = c(8),
    "cv_auc_binaer_global" = c(8),
    "cv_macro_f1" = c(8),
    "cv_accuarcy_balanced" = c(8),
    "cv_accuarcy" = c(8),
    "cv_kappa" = c(8),
    "cv_accuracy_bin" = c(8),
    "cv_accuracy_balanced_bin" = c(8),
    "cv_f1_bin" = c(8),
    "cv_sens_bin" = c(8),
    "cv_spec_bin" = c(8)
  )
performance_final_model_multi[1, ] <-
  cross_validation_multinom_final(10, final_model_multi_formula, lw_et_merged, "verlaufsklasse")
# Performance des Modells ausgeben
performance_final_model_multi
...
stars_ref <-
  star.nominal(
    formula = final_model_multi_formula,
    symmetric = FALSE,
    select = 2:5,
    nlines = 1,
    dist.cov = 1.5,
...
final_model_multi_rnd_eff <-
  mblogit(final_model_multi_formula,
          data = lw_et_merged,
          random = ~ 1 | betrieb)
# Summary des Modells ausgeben
summary(final_model_multi_rnd_eff)
...
performance_final_model_multi_rnd_eff[1, ] <-
  cross_validation_multinom_rnd_effect_final(10, final_model_multi_formula, lw_et_merged, "verlaufsklasse")
# Performance des Modells ausgeben
performance_final_model_multi_rnd_eff
