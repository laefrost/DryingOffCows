...
cross_validation_bin_final <- function(k, formula, data, target) {
  ...
  for (i in 1:k) {
    ...
    pred_probs_global <-
      c(pred_probs, rep(1, nrow(lw_et_merged_af)))
    pred_class_global <-
      c(pred_class, rep("ja", nrow(lw_et_merged_af)))
    cv_auc_global[i] <-
      roc(test_global_antibiotikagabe, pred_probs_global)$auc[1]
  ...
  c(
    ...
    mean(cv_f1, na.rm = T),
    mean(cv_accuracy_balanced, na.rm = T),
    mean(cv_accuracy, NA.rm = T),
    mean(cv_sens, na.rm = T),
    mean(cv_spec, na.rm = T)
  )
}
...
## Formel für den Plot aus dem Bericht (last_milch und log_zellzahl sind hier umgedreht)
...
final_model_bin <-
  glm(final_model_bin_formula,
      data = lw_et_merged,
      family = "binomial"(link = "logit"))

# Modell printen
summary(final_model_bin)
# ---------------------------- Performance schätzen
performance_final_model_bin <-
  data.frame(
    "formula" = c(8),
    "cv_auc" = c(8) ,
    "cv_auc_global" = c(8),
    ...
    "cv_accuarcy" = c(8),
    "cv_sensitivitaet" = c(8),
    "cv_spezifitaet" = c(8)
  )
performance_final_model_bin[1, ] <-
  cross_validation_bin_final(10, final_model_bin_formula, lw_et_merged, "antibiotikagabe")
# Performance printen
performance_final_model_bin
...
forestplot <-
  plot_model(
    final_model_bin,
    value.offset = .3,
    axis.title = "Exponierte Koeffizientenschätzer",
    sort.est = T,
    axis.lim = c(0.01, 20),
    show.values = TRUE,
    title = "Forest-Plot der Koeffizientenschätzer der Einflussgrößen"
  )#, show.intercept = TRUE)
forestplot
effectplot <- plot_model(
  final_model_bin,
  type = "int",
  mdrt.values = "meansd",
  terms = c("log_zellzahl", "last_milch"),
